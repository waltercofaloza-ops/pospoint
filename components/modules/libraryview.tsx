      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5">
        {books.map(book => (
          <div key={book.id} className="bg-card border border-border rounded-2xl overflow-hidden flex flex-col hover:shadow-xl transition hover:border-blue-400">
            <div className="h-40 bg-blue-900 flex items-center justify-center p-4 text-blue-300">
                LIBRERÍA
            </div>
            <div className="p-4 flex-1 flex flex-col">
              <span className="text-xs font-medium text-blue-400 bg-blue-950 px-2.5 py-0.5 rounded-full self-start mb-2">
                {book.genre}
              </span>
